...
def main():
    ...
    for date in dates:
        ...
        if not args.dry_run:
            container_name = f"batch_ing_{date}_{time_ns()}"
            subprocess.run(['docker', 'run', '-d', f'--name={container_name}', f'--env-file={base_env}', '-e', f'CUSTOM_DATE={date}', container], check=True, stderr=subprocess.STDOUT)
